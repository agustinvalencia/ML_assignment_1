---
title: "Machine Learning Assignment 1 - exam prep"
author: "Agustín Valencia - aguva779"
output: pdf_document
---

```{r setup, include=FALSE, warning=FALSE, echo=FALSE}
knitr::opts_knit$set(root.dir = "C:/Users/valen/OneDrive/Documents/Sem01/P02/Machine_Learning/Assignments/ML_assignment_1")
...
```{r}
machines <- read.xlsx("data/machines.xlsx")
machines <- machines[,1]
```

2. Assume the probability model $p(x\mid\theta) = \theta e^{-\theta x}$ for x = Length in which observations are indepentent and identically distributed. What is the distribution type of x? Write a function that computes the log-likelihood $\log p(x\mid\theta)$ for a given data vector $x$. Plot the curve showing the dependence of log-likelihood on $\theta$ where the entire data is used for fitting. 
...
Given the model $p(x|\theta) = \theta e^{-\theta x}$, it can be said that it follows an exponential distribution.

```{r}
llik <- function (theta, data) {
  #res <- log(theta) - (theta * sum(data))
  #res <- log(theta * prod(exp(-theta * data)))
  res <- log(prod(theta * exp(-theta * data)))
  return(res)
}

testThetas <- seq(0, 5, by=.01)
loglik1 <- vector(length = length(testThetas))

for(i in 1:length(testThetas)) {
  loglik1[i] <- llik(testThetas[i], machines)
}

plot(x = testThetas, y = loglik1)

optIdx <- which.max(loglik1)
maxllik <- loglik1[optIdx]
bestTheta <- testThetas[optIdx]

cat("The maximum log-likelihood =",maxllik,"is given at theta=", bestTheta)
```
...
loglik2 <- vector(length = length(testThetas))
for (i in 1:length(testThetas)) {
    loglik2[i] <- llik(testThetas[i], machines[1:6])
}

optIdx <- which.max(loglik2)
maxllik <- loglik2[optIdx]
bestTheta <- testThetas[optIdx]

cat("The maximum log-likelihood =", maxllik, "is given at theta=", bestTheta)

ggplot() + geom_point(aes(x = testThetas, y = loglik1, color = "all data")) +
    geom_point(aes(x = testThetas, y = loglik2, color = "6 obs")) +
    ggtitle("Log-likelihoods comparison") + ylab("log-likelihood") +
    xlab("theta")
```

Since the second likelihood experiment is obtained from a little amount of observations we cannot rely too much on it. It is observed that the maximum is less certain than the curve with all the data. Likelihood estimation will always depend on the amount of data we have available, the more the data the best we can estimate.

4. Assume now a Bayesian model with $p(x\mid\theta)=\theta e ^{-\theta x}$ and a prior $p(\theta) = \lambda e ^ {-\lambda\theta}, \lambda=10$. Write a function computing $l(\theta) = \log(p(\mathbf{x}\mid\theta)p(\theta))$. What kind of measure is actually computed by this function? Plot the curve showing the dependence of $l(\theta)$ on $\theta$ computed using the entire data and overlay it with plot from step2. Find an optimal $\theta$ and compare your result with previous findings.
...
logPosterior <- function(theta, data) {
    ...
    likelihood <- prod(theta * exp(-theta * data))
    posterior <- likelihood * prior
    return(log(posterior))
}

loglik3 <- vector(length = length(testThetas))
for (i in 1:length(testThetas)) {
    loglik3[i] <- logPosterior(testThetas[i], machines)
}

ggplot() + ggtitle("Posterior and likelihood comparison") +
    geom_point(aes(x = testThetas, y = loglik3, color = "posterior")) +
    geom_point(aes(x = testThetas, y = loglik1, color = "likelihood")) +
    xlab("theta") + ylab("P")

optIdx <- which.max(loglik3)
maxllik <- loglik3[optIdx]
bestTheta <- testThetas[optIdx]

cat("The maximum log-likelihood =", maxllik, "is given at theta=", bestTheta)
```

This function is retrieving the Bayesian posterior probability as given by

$$P(H \mid E) = \frac{P(H) P(E \mid H)}{P(E)} = \frac{P(H) P(E \mid H)}{P(H)P(E \mid H) + P(\bar{H}) P(E \mid \bar{H})}$$

![This explanation has been taken from 3Blue1Brown youtube channel](C:\Users\valen\OneDrive\Documents\Sem01\P02\Machine_Learning\Assignments\ML_assignment_1\img\Bayes_explanation.png)

5. Use $\theta$ value found in step 2 and generate 50 new observations from $p(x\mid\theta)=\theta e ^{-\theta x}$ (use standard random number generators). Create the histograms of the original and the new data and make conclusions. 


```{r, out.height='30%', fig.align='center'}
optTheta <- 1.13
newData <- rexp(50, optTheta)
hist(newData)
hist(machines)
```

It can be seen that randomly generated data with the estimated parameters for the real data looks like the real data, which is indicator of a good approximation.
...
3.1. Implement an R function that performs feature selection (best subset selection) in linear regression by using k-fold cross-validation without using any specialized function like lm() (**use only basic R functions**) your function should depend on: 
...
3.2. Test your function on data set swiss available in the standard R repository: 
...
4.1. Import data and create a plot of Moisture versus Protein. Do you think these data are described well by a linear model? 
...
4.2. Consider model $M_i$ in which Moisture is normally distributed and the expected Moisture is polynomial 
...
4.4. Perform variable selection of a linear model in which Fat is response and Channel1-Channel100 are predictors by using stepAIC. Comment on how many variables were selected.
...
4.6. Repeat step 5 but fit with LASSO instead of the Ridge regression and compare the plots from steps 5 and 6. Conclusions? 
