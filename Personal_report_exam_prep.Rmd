...
1.2. Use logistic regression to classify the training and test data by the classification principle $\hat{Y} = 1$ if $p(Y=1 | X ) > 0.5$, otherwise $\hat{Y} = 0$ and report the confusion matrices and the misclassification rates for train and test data. Analyze the obtained results. 
...
1.3. Use logistic regression to classify the test data by the classification principle  $\hat{Y} = 1$ if $p(Y=1 | X ) > 0.8$, otherwise $\hat{Y} = 0$ 
...
1.4. Use standard kknn() with K = 30 from package *kknn*, report the misclassification rates for the training and test data and compare the results with step 2.
...
1.5. Repeat step 4 for K=1 and compare results with step 4. What effects does the decrease of K lead to and why?
...
cat("The maximum log-likelihood is given at theta=", max())
```


3. Repeat step 2 but use only 6 first observations from the data, and p
